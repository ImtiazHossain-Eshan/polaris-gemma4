...
function UsagePanel() {
  return <div className="space-y-4"><div className="grid sm:grid-cols-3 gap-3"><Metric n="Gemma 4" label="Only generative model" /><Metric n="7" label="AI product surfaces" /><Metric n="100%" label="Traceable model routes" /></div><div className="overflow-hidden rounded-xl ring-1 ring-inset ring-polaris-500/10"><table className="w-full text-left text-[12px]"><thead className="bg-paper-soft text-ink-muted"><tr><th className="px-3 py-2">Surface</th><th className="px-3 py-2">Gemma role</th><th className="px-3 py-2">Status</th></tr></thead><tbody className="divide-y divide-polaris-500/10">{[["Strategist","Grounded reasoning and cited action plan"],["Roadmap","Structured diagnosis and milestones"],["Decision Twin","Constraint comparison"],["Evidence Graph","Claim and proof audit"],["Mock review","Personalized mistake analysis"],["Smart Routine","Natural language schedule parsing"],["Offer radar","Official-page synthesis"]].map(([surface, role]) => <tr key={surface}><td className="px-3 py-2 font-semibold text-ink">{surface}</td><td className="px-3 py-2 text-ink-dim">{role}</td><td className="px-3 py-2"><Tag tone="aurora">Gemma 4</Tag></td></tr>)}</tbody></table></div></div>;
}
...
function DataPanel({ profile, memory }: { profile: DemoProfile; memory: MemoryFact[] }) {
  ...
  function exportData() { const blob = new Blob([JSON.stringify({ exportedAt: new Date().toISOString(), profile, memory }, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "polaris-demo-data.json"; a.click(); URL.revokeObjectURL(url); }
  function clear() { [PROFILE_KEY, MEMORY_KEY, "polaris.prefs.notify", "polaris.prefs.marketplace"].forEach((key) => localStorage.removeItem(key)); setCleared(true); }
